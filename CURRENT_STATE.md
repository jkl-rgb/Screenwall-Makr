...
- **Amada-style punch tape import (2026-08):** foreign turret-punch tapes now import — `%` tape markers, bare `X/Y[T]` hit blocks (modal axes, attached tool words), `G92` preset, `G70`/`G72` no-punch positioning, pattern macros **G36 grid / G28 line / G26 bolt circle**, and tool geometry (ROUND / OBROUND / SQUARE) recovered from the **SET-UP SHEET** after the closing `%`. Unknown machine codes are reported, not fatal; unknown tools fall back to a 0.25″ round with a warning. Fixture + 20 tests in `tests/test_amada_punch.py`.
- **Corner fold clearance (2026-08):** at every blank corner where two flanges fold, each flange end edge insets `thickness/2` (total gap = `thickness`, 1:1 with gauge; 3/32″ per side on 3/16″ 3003) so folded edges never clash. Recessed end edges meet in a clean 90° inside corner (no tooth at the hard-corner datum); J+J miters offset perpendicular for a uniform lip gap; RT skew corners handled in vector form. CSV `corner_gap_override` sets total gap per row (0 disables). J slot stations trim for the shortened lip. Regression: `tests/fixtures/outline_golden.json` + `tests/test_corner_fold.py` prove byte-parity when disabled. Also fixed RT-bottom `br`/`tr` outline arrivals (previously drew a stray diagonal through those corner voids).
