...
- **J lip slots:** Shop rule in code: **(slot center to blank outer lip, OC) = nominal CSV F2 − (hole center to inner finished-face edge, OC)** (`_j_slot_axis_coord_shop_ortho`, nominal from `flange2_depth` or MIX `*_f2`). Stations use only the **outermost** perforation row/column for that flange (stagger-safe); along-flange spacing is never **greater** than **12″** c–c (`FASTENING_J_MAX_OC_IN`); tighter when perforations require. **J+J miters:** along-flange x/y limits use the **void-inner HC quad plus miter depth `fh2`/`fv2`** (same as the `cut` outline short base between miter ends), inset by **half slot length** so slot geometry does not enter the miter triangle. RT uses the same rule when the lip run is axis-aligned (`_rt_j_miter_span_*`). Normal position is clamped inside the developed lip (`SideDef.f2`). RT J slots use the same nominal-F2 rule with finished-face edge sampled at each hole (`_j_slot_normal_rt`).
