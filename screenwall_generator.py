...
def _blank_outline(blank_w, blank_h, sides, bd):
    ...
    def _corner(hsd, vsd, hc_x, hc_y, ox, oy):
        """
        Returns points for this corner in CCW order.
        Includes the arriving primary edge endpoint as first point
        and the departing primary edge startpoint as last point.
        ox, oy: outward direction signs (toward blank edge)
          BL: ox=-1, oy=-1   BR: ox=+1, oy=-1
          TR: ox=+1, oy=+1   TL: ox=-1, oy=+1
        """
        ...
        # Key points
        # h_void_end: end of void edge along horiz direction (down/up)
        h_void = (hc_x,         hc_y + oy*fh1)       # down/up by fh1
        # h_blank: end of miter on blank edge (or h_void if L)
        h_blank= (hc_x+(-ox)*fh2, hc_y+oy*(fh1+fh2)) # miter end on blank edge
        # v_void_end: end of void edge along vert direction (left/right)
        v_void = (hc_x + ox*fv1, hc_y)               # left/right by fv1
        # v_blank: end of miter on blank edge (or v_void if L)
        v_blank= (hc_x+ox*(fv1+fv2), hc_y+(-oy)*fv2) # miter end on blank edge

        h_act=hsd.active; v_act=vsd.active
        h_J=(h_act and fh2>0); h_L=(h_act and fh2==0)
        v_J=(v_act and fv2>0); v_L=(v_act and fv2==0)

        # Build point sequence: arrive from vert primary → depart to horiz primary
        # (CCW means: arriving going in -oy direction on vert side, departing in -ox on horiz)
        pts = []

        if not h_act and not v_act:
            pts = [hc]

        elif h_act and not v_act:
            # J+N or L+N: corner is at face boundary, only horiz flange geometry
            if h_J:
                pts = [hc, h_void, h_blank]
            else:  # L+N
                pts = [hc, h_blank]  # h_blank == h_void for L (fh2=0)

        elif not h_act and v_act:
            # N+J or N+L
            if v_J:
                pts = [hc, v_void, v_blank]  # wait: wrong order for CCW
                # arriving from vert primary... but vert is active and horiz not
                # arrive from vert primary end → corner → no horiz primary → go to next vert
                # Actually: arrive at v_blank (top of vert miter on blank edge)
                # then v_void, then HC, then... nothing (horiz inactive)
                # Departing is just the HC (face corner)
                pts = [v_blank, v_void, hc]
            else:  # N+L
                pts = [v_blank, hc]  # v_blank==v_void for L

        else:
            # Both active: J+J, L+L, J+L, L+J
            # Sequence CCW (arriving from vert primary, departing to horiz primary):
            # v_blank/v_void → v_void/hc → HC → h_void/hc → h_blank/h_void
            if v_J and h_J:   # J+J: 4 edges, 5 pts
                pts = [v_blank, v_void, hc, h_void, h_blank]
            elif v_L and h_J:  # L+J: 3 edges, 4 pts
                pts = [v_blank, hc, h_void, h_blank]
            elif v_J and h_L:  # J+L: 3 edges, 4 pts
                pts = [v_blank, v_void, hc, h_blank]
            else:              # L+L: 2 edges, 3 pts
                pts = [v_blank, hc, h_blank]

        return pts

    # Build CCW polygon.
    # Each _corner() returns points AFTER the arriving primary endpoint.
    # The arriving primary endpoint (v_blank) is prepended before each corner.
    # This ensures each edge is represented exactly once.

    def _v_blank(vsd, hc_x, hc_y, ox, oy):
        """Compute the arriving primary endpoint (v_blank) for a corner."""
        fv1 = _ve(vsd); fv2 = _md(vsd)
        if not vsd.active:
            return (hc_x, hc_y)  # no vert flange: arrive at HC
        return (hc_x + ox*(fv1+fv2), hc_y + (-oy)*fv2)

    bl = _corner(sb, sl, fx0, fy0, ox=-1, oy=-1)
    br = _corner(sb, sr, fx1, fy0, ox=+1, oy=-1)
    tr = _corner(st, sr, fx1, fy1, ox=+1, oy=+1)
    tl = _corner(st, sl, fx0, fy1, ox=-1, oy=+1)

    bl_arr = _v_blank(sl, fx0, fy0, ox=-1, oy=-1)
    br_arr = _v_blank(sr, fx1, fy0, ox=+1, oy=-1)
    tr_arr = _v_blank(sr, fx1, fy1, ox=+1, oy=+1)
    tl_arr = _v_blank(sl, fx0, fy1, ox=-1, oy=+1)

    pts = []
    def _add(arrive, cpts):
        # Only add arrive if different from first corner point (avoids duplicates)
        if not cpts or abs(arrive[0]-cpts[0][0])>1e-9 or abs(arrive[1]-cpts[0][1])>1e-9:
            pts.append(arrive)
        pts.extend(cpts)
