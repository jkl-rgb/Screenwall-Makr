...
- `shop_flat_mode`: `auto` (default) uses the shop bend deduction — **0.335″ per 90° bend at 3/16″**, constant across alloys, scaling 1:1 with thickness (job 78060 / WT-1.01 calibration). Per side: **L leg = F1 − BD**, **J leg = F1 − 1.5·BD**, **J lip = F2 − 0.5·BD** (face keeps full nominal). A 2″ L develops 1.665″ (1.860″ from the hard inside corner incl. the 0.195″ datum). `off` uses pure bend theory (`_flat_leg_L` / `_flat_leg_J` / `_flat_lip`).
